...
/* ------------------------------ Navigation ------------------------------ */
/* Icônes de la barre de navigation — chargées depuis assets/img/icons/.
   Dépose tes propres images dans ce dossier (même nom de fichier) sur GitHub
   pour les remplacer : aucune modification de code n'est nécessaire. */
const ICONS_PATH = 'assets/img/icons/';
const TABS = [
  {id:'home', label:'Accueil', icon: ICONS_PATH + 'home.png'},
  {id:'meals', label:'Repas', icon: ICONS_PATH + 'meals.png'},
  {id:'recipes', label:'Recettes', icon: ICONS_PATH + 'recipes.png'},
  {id:'shopping', label:'Courses', icon: ICONS_PATH + 'shopping.png'},
  {id:'settings', label:'Réglages', icon: ICONS_PATH + 'settings.png'}
];

function buildNavHTML(activeId){
  return TABS.map(t => `
    <button class="navitem ${t.id===activeId?'active':''}" data-nav="${t.id}">
      <img src="${t.icon}" alt="" class="navicon">
      ${t.label}
